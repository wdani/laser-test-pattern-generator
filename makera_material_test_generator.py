#!/usr/bin/env python3
"""
Makera Material Test Generator v1.0 experimental
...
def nc_header(settings: GeneratorSettings) -> List[str]:
    return [
        "; Generated by Makera Material Test Generator v1.0",
        "; Generic NC/G-code output. Verify your controller's laser S-value scale before use.",
        f"; S max = {settings.nc_s_max:g}; 100% power maps to S{settings.nc_s_max:g}",
        "G21 ; millimeters",
...
def nc_laser_line(lines: List[str], x1: float, y1: float, x2: float, y2: float, speed: float, power: float, s_max: float) -> None:
    ...
    lines.append(f"M3 S{fmt_num(s_value, 3)}")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--nc-s-max", type=float, default=1000.0, help="Generic NC S-value for 100 percent power")
...
class GeneratorGui:
    def __init__(self) -> None:
        ...
        self.root = tk.Tk()
        self.root.title("Makera Material Test Generator v1.0")
    ...
    def _build_laser_tab(self):
        ...
        self._entry(f, "NC S max", "nc_s_max", "1000", 4, 0)
        note = ttk.Label(f, text="Generic NC only: 100% power maps to S max. Common values are 1, 255, 1000, 10000.", foreground="#555")
        note.grid(row=5, column=0, columnspan=4, sticky="w", padx=6, pady=10)
        ...
        try:
            ...
            for info in infos:
                ...
                if info.get("format") == "NC":
                    self._log(f"NC lines: {info['lines']} | Tiles: {info['tiles']} | S max: {info['s_max']}")
                else:
